...
//    Con persistencia en localStorage para sobrevivir F5/recargas
// ═══════════════════════════════════════════════════════════════════════════

const WD_STORAGE_KEY   = 'wd_state_v2';
const ITER_STORAGE_KEY = 'iter_runner_v1';
const WD_INTERVAL_MS   = 30 * 60 * 1000;
const WD_BUFFER_MS     =  2 * 60 * 1000;
const ITER_INTERVAL_MS =  5 * 60 * 1000;
const ITER_BUFFER_MS   = 30 * 1000;

// ── Helpers de persistencia ───────────────────────────────────────────────────

function wdSaveState() {
  try {
    localStorage.setItem(WD_STORAGE_KEY, JSON.stringify({
      lastRunAt:  _wd.lastRunAt,
      nextRunAt:  _wd.nextRunAt,
      lastResult: _wd.lastResult,
      savedAt:    Date.now(),
    }));
  } catch(_) {}
}

function wdLoadState() {
  try {
    const raw = localStorage.getItem(WD_STORAGE_KEY);
    if (!raw) return null;
    const s = JSON.parse(raw);
    // Descartar si tiene más de 2 horas de antigüedad
    if (!s.savedAt || (Date.now() - s.savedAt) > 2 * 3600 * 1000) return null;
    return s;
  } catch(_) { return null; }
}

function iterSaveState(lastRunAt) {
  try {
    localStorage.setItem(ITER_STORAGE_KEY, JSON.stringify({ lastRunAt, savedAt: Date.now() }));
  } catch(_) {}
}

function iterLoadState() {
  try {
    const raw = localStorage.getItem(ITER_STORAGE_KEY);
    if (!raw) return null;
    const s = JSON.parse(raw);
    if (!s.savedAt || (Date.now() - s.savedAt) > 3600 * 1000) return null;
    return s;
  } catch(_) { return null; }
}
...
async function _wdRun(manual = false) {
  ...
  try {
    ...
    _wd.running   = false;
    _wd.nextRunAt = Date.now() + WD_INTERVAL_MS;
    wdSaveState(); // ← persistir en localStorage
    _wdRender();
  }
}
...
function startWatchdog() {
  ...
  // ── Recuperar estado anterior de localStorage ────────────────────────────
  const saved = wdLoadState();
  if (saved) {
    _wd.lastRunAt  = saved.lastRunAt  || null;
    _wd.lastResult = saved.lastResult || null;
    _wd.nextRunAt  = saved.nextRunAt  || null;
    console.log('[watchdog] Estado recuperado — lastRunAt:',
      _wd.lastRunAt ? new Date(_wd.lastRunAt).toLocaleTimeString() : 'nunca');
  }
  ...
  // ── Scheduler principal ───────────────────────────────────────────────────
  _wd.interval = setInterval(() => _wdRun(), WD_INTERVAL_MS);

  // ── Decidir cuándo ejecutar la primera vez ────────────────────────────────
  const now              = Date.now();
  const timeSinceLastRun = _wd.lastRunAt ? (now - _wd.lastRunAt) : Infinity;

  if (timeSinceLastRun < (WD_INTERVAL_MS - WD_BUFFER_MS)) {
    // Corrió recientemente — calcular tiempo restante y esperar
    const waitMs  = Math.max(5000, WD_INTERVAL_MS - timeSinceLastRun);
    _wd.nextRunAt = now + waitMs;
    console.log(
      `[watchdog] 🔔 Iniciado — corrió hace ${Math.round(timeSinceLastRun/60000)}min. ` +
      `Próxima en ${Math.round(waitMs/60000)}min`
    );
    setTimeout(() => {
      _wdRun();
    ...
    // No ha corrido en mucho tiempo (o nunca) — ejecutar en 15s
    _wd.nextRunAt = now + 15000;
    console.log('[watchdog] 🔔 Iniciado — primera ejecución en 15s');
...
// ═══════════════════════════════════════════════════════════════════════════
// 5. AUTO-RUNNER DE ITERACIONES — cada 5 minutos, con catch-up al volver
// ═══════════════════════════════════════════════════════════════════════════
...
async function runPendingIterationsBackground() {
  try {
    ...
    const d = await r.json();
    if (d.success) {
      iterSaveState(Date.now()); // ← persistir timestamp
      if (d.count > 0) {
...
function startIterationRunner() {
  ...
  const saved         = iterLoadState();
  const now           = Date.now();
  const timeSinceLast = saved?.lastRunAt ? (now - saved.lastRunAt) : Infinity;
  const needsCatchup  = timeSinceLast > (ITER_INTERVAL_MS + ITER_BUFFER_MS);

  _iterationRunnerInterval = setInterval(runPendingIterationsBackground, ITER_INTERVAL_MS);

  if (needsCatchup) {
    const minsAgo = timeSinceLast === Infinity ? 'nunca' : `${Math.round(timeSinceLast/60000)}min`;
    console.log(`[iter-runner] Auto-runner iniciado — sin correr desde hace ${minsAgo} — catch-up inmediato`);
    setTimeout(runPendingIterationsBackground, 2000);
  } else {
    const waitMs = Math.max(5000, ITER_INTERVAL_MS - timeSinceLast);
    console.log(`[iter-runner] Auto-runner iniciado — corrió hace ${Math.round(timeSinceLast/60000)}min. Próxima en ${Math.round(waitMs/60000)}min`);
    setTimeout(runPendingIterationsBackground, waitMs);
  }
}
...
document.addEventListener('visibilitychange', () => {
  ...
  // WD: ¿toca ejecutar?
  const wdTimeSince = _wd.lastRunAt ? (now - _wd.lastRunAt) : Infinity;
  if (wdTimeSince >= (WD_INTERVAL_MS - WD_BUFFER_MS)) {
    console.log('[watchdog] Tab activa de nuevo — WD vencido, ejecutando en 2s');
    setTimeout(() => _wdRun(false), 2000);
  }

  // Iteraciones: ¿hay pendientes?
  const iterSaved     = iterLoadState();
  const iterTimeSince = iterSaved?.lastRunAt ? (now - iterSaved.lastRunAt) : Infinity;
  if (iterTimeSince >= (ITER_INTERVAL_MS - ITER_BUFFER_MS)) {
    console.log('[iter-runner] Tab activa — posibles iteraciones pendientes, ejecutando en 3s');
